...
private val pattern = Pattern.compile("move (?<$TIMES>\\d+) from (?<$FROM>\\d+) to (?<$TO>\\d+)")
    ?: throw RuntimeException("Invalid pattern")
...
fun processInput(lines: List<String>): Pair<ContainerStack, List<Move>> {
    ...
    val containerStack = ContainerStack(
        lines
            .subList(0, splitIndex - 1)
            .map { line ->
                List(numberOfStacks) { line.getOrElse(it * 4 + 1) { ' ' } }
            }
