...
fun main() {
    val lines = InputReader().readInput("2022", "3")

    /** ****** Part 1 ******* */
    lines
      .map { it.subSequence(0..it.length / 2) to it.subSequence(it.length / 2 until it.length) }
    ...
    lines
      ...
      .map { triple -> triple.first.filter { triple.second.contains(it) }.first { triple.third.contains(it) } }
      .onEach { println(it) }
      .sumOf { priority(it) }
      .let { println(it) }
}
