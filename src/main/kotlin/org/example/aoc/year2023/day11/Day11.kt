    ...
    companion object {
        fun List<List<Boolean>>.findEmptyRows(): Set<Int> {
            return this.withIndex()
                .filter { (_, row) -> row.all { element -> !element } }
                .map { (rowIndex, _) -> rowIndex }
                .toSet()
        }
        ...
        fun List<List<Boolean>>.transpose(): List<List<Boolean>> {
            return this.fold(initial = List(this.size) { emptyList() }) { acc, row ->
                acc.zip(row).map { (accRow, element) -> accRow.plus(element) }
            }
        }
        ...
        fun List<List<Boolean>>.duplicateEmptyRowsAndColumn(): List<List<Boolean>> {
            ...
            return this
                .duplicateRows(emptyRows)
                .duplicateColumns(emptyColumns)
        }
...
//            return this.transpose().doubleRows(columnsToDouble).transpose()
        ...
        fun List<List<Boolean>>.getAllTrue(): Set<PositionInt> {
            ...
            }.filter { (_, element) -> element }.map { (position, _) -> position }.toSet()
        }
        ...
        fun Set<PositionLong>.duplicateRows(rowsToDuplicate: Set<Int>, timesToDuplicate: Int): Set<PositionLong> {
            return this.map { eachPosition ->
                val numberOfRowsToShift =
                    rowsToDuplicate.count { each -> eachPosition.row > each }.toLong().times(timesToDuplicate-1)
        ...
        fun Set<PositionLong>.duplicateColumns(columnsToDuplicate: Set<Int>, timesToDuplicate: Int): Set<PositionLong> {
            return this.map { eachPosition ->
                val numberOfColumnsToShift =
                    columnsToDuplicate.count { each -> eachPosition.column > each }.toLong().times(timesToDuplicate-1)
                eachPosition.plus(column = numberOfColumnsToShift)
            }.toSet()
        }

        data class PositionInt(val row: Int, val column: Int) {
            fun manhattanDistance(other: PositionInt): Int =
                (this.row - other.row).absoluteValue + (this.column - other.column).absoluteValue
        ...
        data class PositionLong(val row: Long, val column: Long) {
            fun manhattanDistance(other: PositionLong): Long =
                (this.row - other.row).absoluteValue + (this.column - other.column).absoluteValue
