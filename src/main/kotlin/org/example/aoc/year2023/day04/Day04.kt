...
fun main() {
    val lines = InputReader().readInput("2023", "04")
    val day4 = Day4()
...
class Day4 {
    ...
    fun part2(lines: List<String>): Int {
        ...
        return cards
          .foldIndexed(List(cards.size) { 1 }) { index: Int, numberOfCardList: List<Int>, currentCard: Card ->
...
data class Card(val id: Int, val winningNumber: Set<Int>, val number: Set<Int>) {
    val matching: Int = number.intersect(winningNumber).size

    fun getPoint(): Int = if (matching == 0) 0 else (2.0).pow(matching - 1).toInt()
}
