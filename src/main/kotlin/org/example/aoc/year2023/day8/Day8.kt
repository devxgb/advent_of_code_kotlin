...
class Day8 {
    ...
    fun part2(lines: List<String>): Long {
        ...
        val graphTraversals = graph.getVerticesWithOutgoingEdge()
            .filter { it.endsWith('A') }
            .map { node -> GraphTraversal(graph, node, parseInstructions) { it.endsWith('Z') } }

        return graphTraversals.map {
            while (it.isEnd().not()) {
                it.moveNext()
            }
            it.stepCount
