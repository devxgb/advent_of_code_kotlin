...
class Day2 {
    fun part1(lines: List<String>): Int {
        ...
        val totalGreen = 13
        val totalBlue = 14
        return lines.map { line -> Game.fromString(line) }.filter { game -> game.isPossible(totalRed, totalGreen, totalBlue) }.sumOf { it.id }
    }
    ...
    fun part2(lines: List<String>): Int =
      lines
        ...
        .sumOf { turn -> turn.red * turn.green * turn.blue }
}
        ...
        fun fromString(str: String): Game {
            ...
            val parsedTurns =
              turns
                ...
                .map { turn -> Turn(turn["red"] ?: 0, turn["green"] ?: 0, turn["blue"] ?: 0) }

            return Game(id, parsedTurns)
        }
    }

    fun isPossible(totalRed: Int, totalGreen: Int, totalBlue: Int): Boolean = turns.all { it.isPossible(totalRed, totalGreen, totalBlue) }
}

class Turn(val red: Int = 0, val green: Int = 0, val blue: Int = 0) {
    fun isPossible(totalRed: Int, totalGreen: Int, totalBlue: Int): Boolean = red <= totalRed && green <= totalGreen && blue <= totalBlue
}
